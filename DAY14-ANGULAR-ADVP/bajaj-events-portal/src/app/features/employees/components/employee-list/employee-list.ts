...
export class EmployeeList implements OnInit, OnDestroy {
  ...
  protected title = 'Welcome to Bajaj finserv Employee List Component!';
  protected subTitle = 'Subtitle - Welcome to Bajaj finserv Employee List Component!';
  protected columns: string[] = ['employee id', 'employee name', 'city', 'phone', 'show details'];
  protected childSubtitle = 'Details of selected employee!';
  protected employeeChildmessage = '';
  ...
  ngOnInit(): void {
    this._employeeservicesubsription = this._employeeApi.getAllEmployees().subscribe({
      next: (empData) => {
        console.log('Raw data:', empData);
        // Filter out invalid records
        this.employee = empData.filter(emp => emp.employeeId && emp.employeeName);
        this.filteredEmployee = [...this.employee];
        console.log('Filtered employees:', this.filteredEmployee);
      },
  ...
  ngOnDestroy(): void {
    if (this._employeeservicesubsription) {
      this._employeeservicesubsription.unsubscribe();
    }
  }
  ...
  protected searchEmployee(): void {
    const searchTerm = this.searchChars.trim().toLowerCase();
    this.filteredEmployee = searchTerm
      ? this.employee.filter(emp => emp.employeeName?.toLowerCase().includes(searchTerm))
      : [...this.employee];
  }
